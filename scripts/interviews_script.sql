...
SELECT
    ...
    MAX(a.admission_date) AS admission_date,
...
SELECT
	a.patient_id,
    a.patient_id||CAST(LEN(p.last_name) AS INT)||CAST(YEAR(p.birth_date) AS INT) AS temp_password
FROM patients AS p 
JOIN admissions AS a 
ON p.patient_id = a.patient_id
...
SELECT
	...
	ROUND(SUM(d.discount * d.quantity * p.unit_price),2) discount_amount
